package org.example;
import org.example.ValorFueradeRangoExcepcion;
import org.example.TamanoFueradeRangoExcepcion;
...
public class ProductoAdyacentes {
    ...
    public static void validarValores(int[] arreglo) throws ValorFueradeRangoExcepcion {
        for (int i = 0; i < arreglo.length; i++) {
            if (arreglo[i]<-1000 || arreglo[i]>1000){
                throw new ValorFueradeRangoExcepcion("El arreglo debe tener valores que no sean menores a -1000 y mayores a 1000");
            }
        }
    }
    public static void validarArreglo(int[] arreglo) throws TamanoFueradeRangoExcepcion {}   {
        if (arreglo.length <=2 || arreglo.length>=20){
            throw new IllegalArgumentException("El arreglo debe tener entre 2 y 20 elementos");
        }
    }
